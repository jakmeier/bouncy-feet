...
mod tests {
    ...
    async fn setup_client_session(pool: &PgPool) -> (UserId, i64, Uuid) {
    ...
    async fn authenticate_guest_session_succeeds_with_valid_secret(
        pool: PgPool,
    ) -> sqlx::Result<()> {
        ...
        assert!(
            result.is_some(),
            "authentication should succeed with valid secret"
        );
    ...
    #[sqlx::test(migrations = "./db_migrations")]
    async fn authenticate_guest_session_fails_with_invalid_secret(
        pool: PgPool,
    ) -> sqlx::Result<()> {
        ...
        let result =
            ClientSessionId::authenticate_guest_session(&pool, session_id, &wrong_secret).await;

        assert!(
            result.is_none(),
            "authentication should fail with invalid secret"
        );
        Ok(())
    }

    #[sqlx::test(migrations = "./db_migrations")]
    async fn authenticate_guest_session_fails_with_nonexistent_session(
        pool: PgPool,
    ) -> sqlx::Result<()> {
        let secret = Uuid::new_v4();
        ...
        let result =
            ClientSessionId::authenticate_guest_session(&pool, nonexistent_session_id, &secret)
                .await;

        assert!(
            result.is_none(),
            "authentication should fail for nonexistent session"
        );
        Ok(())
    }
    ...
    async fn authenticate_guest_session_handles_multiple_sessions_isolation(
        ...
        let result_a_with_a =
            ClientSessionId::authenticate_guest_session(&pool, session_a_id, &secret_a).await;
        let result_a_with_b =
            ClientSessionId::authenticate_guest_session(&pool, session_a_id, &secret_b).await;

        // Session B should authenticate with its own secret only
        let result_b_with_b =
            ClientSessionId::authenticate_guest_session(&pool, session_b_id, &secret_b).await;
        let result_b_with_a =
            ClientSessionId::authenticate_guest_session(&pool, session_b_id, &secret_a).await;

        assert!(
            result_a_with_a.is_some(),
            "session A should authenticate with its secret"
        );
        assert!(
            result_a_with_b.is_none(),
            "session A should not authenticate with session B's secret"
        );
        assert!(
            result_b_with_b.is_some(),
            "session B should authenticate with its secret"
        );
        assert!(
            result_b_with_a.is_none(),
            "session B should not authenticate with session A's secret"
        );
        Ok(())
    }

    #[sqlx::test(migrations = "./db_migrations")]
    async fn authenticate_guest_session_returns_correct_session_id(
        pool: PgPool,
    ) -> sqlx::Result<()> {
        let (_user_id, session_id, secret) = setup_client_session(&pool).await;
        ...
        assert_eq!(
            authenticated.num(),
            session_id,
            "returned session ID should match the input"
        );
        Ok(())
    }
    ...
    #[sqlx::test(migrations = "./db_migrations")]
    async fn transfer_client_sessions_moves_sessions_to_target_user(
        pool: PgPool,
    ) -> sqlx::Result<()> {
        ...
        let source_session_count: i64 =
            sqlx::query_scalar("SELECT COUNT(*) FROM client_session WHERE user_id = $1")
                .bind(source_user.num())
                .fetch_one(&pool)
                .await?;
    ...
    async fn transfer_client_sessions_partial_transfer_only_affects_source(
        ...
        let unrelated_session_count: i64 =
            sqlx::query_scalar("SELECT COUNT(*) FROM client_session WHERE user_id = $1")
                .bind(unrelated_user.num())
                .fetch_one(&pool)
                .await?;
    ...
    async fn transfer_client_sessions_handles_empty_transfer_silently(
        ...
        assert_eq!(
            ...
            "target user should still have no sessions"
        );
        Ok(())
    }
}
